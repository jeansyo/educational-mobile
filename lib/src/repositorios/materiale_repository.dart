...
Future<Map<String,dynamic>> obtenerMateriales(int id) async {
    // Uri uri = Helper.getUriLfr('api/producto');
    String apiToken = await repol.getApiToken();
    try {
      if (apiToken != null && apiToken.isNotEmpty) {
        final String url =
                '${GlobalConfiguration().get('base_url').toString()}materials/$id';
        //print(url);
  ...
  Future<Map<String,dynamic>> obtenerMaterialesRecientes() async {
    // Uri uri = Helper.getUriLfr('api/producto');
    String apiToken = await repol.getApiToken();
    try {
      if (apiToken != null && apiToken.isNotEmpty) {
        final String url =
                '${GlobalConfiguration().get('base_url').toString()}materials/recent';
        //print(url);
        var uri = Uri.parse(url);
        final client = http.Client();
        final response = await client.get(uri,
